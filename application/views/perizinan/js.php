	...
	if ($('#form-perizinan').length) {
		$('#form-perizinan').on('submit', function (event) {
			...
			if (startDate && startDate <= today) {
				$('#error_tanggal_mulai').text('Tanggal mulai harus setelah hari ini.');
				$('#tanggal_mulai').addClass('is-invalid');
				isValid = false;
			}

			if (startDate && endDate && endDate < startDate) {
				$('#error_tanggal_selesai').text('Tanggal selesai tidak boleh sebelum tanggal mulai.');
					...
					$.ajax({
						...
						success: function(response) {
							if (response.status === 'success') {
								...
								Swal.fire({
									icon: 'error',
									title: 'Pengajuan Tidak Dapat Disimpan',
		...
		$('#tanggal_mulai_picker, #tanggal_selesai_picker').datetimepicker({
			...
			// Struktur tombol versi Tempus Dominus
			buttons: { 
				showToday: true, 
				showClear: true, 
				showClose: true 
			},
			// Wajib dipetakan jika menggunakan FontAwesome 5 bawaan AdminLTE v3
			icons: {
				time: 'far fa-clock',
				date: 'fa fa-calendar',
				up: 'fa fa-chevron-up',
		...
		$('#jam_mulai_picker, #jam_selesai_picker').datetimepicker({
			...
			// 1. Menampilkan tombol aksi di bagian bawah
			buttons: { 
				showToday: true, 
				showClear: true, 
				showClose: true 
			...
			// 2. Pemetaan ikon FontAwesome bawaan AdminLTE v3 agar tombolnya muncul
			icons: {
				time: 'far fa-clock',
				date: 'fa fa-calendar',
				up: 'fa fa-chevron-up',
				...
				previous: 'fa fa-chevron-left',
				next: 'fa fa-chevron-right',
				today: 'far fa-calendar-check', // Tombol "Today" (akan mengarah ke jam sekarang)
				clear: 'far fa-trash-alt',     // Tombol "Clear"
				close: 'fa fa-times'            // Tombol "Close"
			}
		});
